...
async function _initClient() {
  ...
  _isConnecting = true;
  try {
    // Use local Docker Redis for production deployment
    let url = process.env.LOCAL_REDIS_URL || process.env.REDIS_URL || 'redis://127.0.0.1:6379';
    let password = process.env.REDIS_PASSWORD || undefined;
    
    console.log('🐳 Using local Redis for production deployment:', url);
