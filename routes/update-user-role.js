...
router.post(
    ...
    async (req, res) => {
        ...
        try {
            ...
            const studentQuery = `
            UPDATE student SET 
                name = ?, date_of_birth = ?, board = ?, school_name = ?,
                class_year = ?, medium = ?, phone_number = ?, address = ?, 
                state = ?, pincode = ?, country = ?, profileimage = ?
            WHERE email = ?
        `;
...
router.post("/update-profile",verifyToken, upload.single("profileimage"), async (req, res) => {
    try {
        ...
        const studentQuerry=`
        UPDATE student SET 
            name = ?, date_of_birth = ?, education_level = ?, board = ?, school_name = ?,
            class_year = ?, medium = ?, phone_number = ?, address = ?, 
            state = ?, pincode = ?, country = ?, profileimage = ?
        WHERE email = ?
        `
        const params =[
            name,date_of_birth,education_level,board,school_name,class_year,medium,phone_number,
            address,state,pincode,country,profileImageUrl,email
        ]
        console.log("1234")
