...
router.post("/teacherInfo", verifyToken, async (req, res) => {
    ...
    try {
            ...
            return teachers.filter(teacher => {
                ...
                if (Array.isArray(tuitions) && tuitions.length > 0) {
                    ...
                    const hasMatch = tuitions.some(tuition => {
                        // Flexible board name matching
                        const boardNameVariations = [
                            board,
                            board?.replace(' Board of Secondary Education', '').replace(' Certificate of Secondary Education', '').replace(' Board', ''),
                            board?.split(' ').map(w => w[0]).join(''), // CBSE, ICSE, UP
                            board?.toLowerCase(),
                            'CBSE',
                            'ICSE',
                            'UP'
                        ];
                        const boardMatch = !board || (tuition.board && (
                            boardNameVariations.includes(tuition.board) || 
                            tuition.board === board ||
                            board?.toLowerCase().includes(tuition.board?.toLowerCase()) ||
                            tuition.board?.toLowerCase().includes(board?.toLowerCase())
                        ));
                        ...
                        const classMatch = !className || (tuition.class && (
                            classNameVariations.includes(tuition.class) ||
                            tuition.class === className ||
                            className?.toLowerCase().includes(tuition.class?.toLowerCase()) ||
                            tuition.class?.toLowerCase().includes(className?.toLowerCase())
                        ));
                        
                        // Flexible subject name matching
                        const subjectMatch = !subject || (tuition.subject && (
                            tuition.subject === subject ||
                            subject?.toLowerCase().includes(tuition.subject?.toLowerCase()) ||
                            tuition.subject?.toLowerCase().includes(subject?.toLowerCase())
                        ));
        ...
        // Deduplicate teachers within the specific board/class/subject context
        // A teacher can appear in different flows (e.g., CBSE Class 7 and ICSE Class 9)
        // But should only appear once within the same flow (e.g., CBSE Class 7 Social Studies)
        const deduplicateTeachers = (teachers) => {
            const seen = new Set();
            return teachers.filter(teacher => {
                if (seen.has(teacher.email)) {
                    return false;
                }
        ...
        const uniqueSpotlight = deduplicateTeachers(filteredSpotlight);
        const uniquePopular = deduplicateTeachers(filteredPopular);
        ...
        const spotlightSkill = uniqueSpotlight
            .filter((t) => t.category === "Skill teacher");

        const spotlightSubject = uniqueSpotlight
