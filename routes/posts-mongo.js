...
function getImageUrl(imagePath) {
  ...
  const baseUrl = process.env.BASE_URL || 'https://growsmartserver.gogrowsmart.com';
...
router.post('/create', verifyToken, async (req, res) => {
  try {
    const { content, tags, postImage } = req.body;
    const userEmail = req.user.email;
    const userRole = req.user.role || 'Unknown';

    console.log('🔍 Creating post - User Role:', userRole, 'Email:', userEmail);
    console.log('📸 Image data:', { hasPostImage: !!postImage, postImageLength: postImage?.length });
    ...
    // Handle base64 image - upload to S3 for production
    let postImagePath = null;
    if (postImage && postImage.startsWith('data:image')) {
      try {
        console.log('📤 Uploading image to S3...');
        postImagePath = await uploadBase64ImageToS3(postImage, postId);
        console.log('✅ Image uploaded to S3:', postImagePath);
      } catch (error) {
        console.error('❌ Error uploading to S3, falling back to local storage:', error);
        // Fallback to local storage if S3 fails
        const filename = `${Date.now()}.jpg`;
        postImagePath = saveBase64Image(postImage, filename);
      }
    }
    
    console.log('🖼️ Post image path:', postImagePath);
    ...
    res.status(201).json({
      ...
      data: {
        ...
        author_profile_pic: userProfile.profile_pic,
        content: content.trim(),
        post_image: getImageUrl(postImagePath),
        likes: 0,
